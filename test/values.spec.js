...
describe('Multiple values', function() {
	...
	it('can take a function to create keys dynamically', function() {
		...
		var dim = data.dimension(function() {
			return 'all';
		});
	...
	it('works with regular value() as well', function() {
		var data = crossfilter([
			{ foo: 'one', x: 1, other: 2 },
			{ foo: 'two', x: 2, other: 1 },
			{ foo: 'three', x: 3, other: 4 },
		...
		debugger;
		var group = reducer(dim.groupAll());

		expect(group.value().Test.one.count).toBe(10);
		expect(group.value().Test.one.sum).toBe(5);
		expect(group.value().Test.one.avg).toBe(0.5);

		expect(group.value().Test.two.count).toBe(8);
		expect(group.value().Test.two.sum).toBe(4);
		expect(group.value().Test.two.avg).toBe(0.5);

		expect(group.value().Test.three.count).toBe(3);
		expect(group.value().Test.three.sum).toBe(4);
		expect(group.value().Test.three.avg).toBe(4 / 3);
	});
});
