...
function build_function(p, f, path) {
  ...
  if (p.value) {
    ...
    f.reduceAdd = reductio_value.add(
      p.value,
      newF,
      path,
      p.multi_value,
  ...
  if (p.avg) {
    if (!p.count || !p.sum) {
      console.error(
        "You must set .count(true) and define a .sum(accessor) to use .avg(true)."
      );
    } else {
  ...
  if (p.exceptionCount) {
    if (!p.exceptionAccessor) {
      console.error(
        "You must define an .exception(accessor) to use .exceptionCount(true)."
      );
    } else {
      f.reduceAdd = reductio_exception_count.add(
        p.exceptionAccessor,
        f.reduceAdd,
        path
      );
      f.reduceRemove = reductio_exception_count.remove(
        p.exceptionAccessor,
        f.reduceRemove,
        path
      );
  ...
  if (p.exceptionSum) {
    if (!p.exceptionAccessor) {
      console.error(
        "You must define an .exception(accessor) to use .exceptionSum(accessor)."
      );
    } else {
      f.reduceAdd = reductio_exception_sum.add(
        p.exceptionAccessor,
        p.exceptionSum,
        f.reduceAdd,
        path
      );
      f.reduceRemove = reductio_exception_sum.remove(
        p.exceptionAccessor,
        p.exceptionSum,
        f.reduceRemove,
        path
  ...
  if (p.valueList || p.median || p.min || p.max) {
    ...
    f.reduceRemove = reductio_value_list.remove(
      p.valueList,
      f.reduceRemove,
      path
    );
  ...
  if (p.dataList) {
    ...
    f.reduceRemove = reductio_data_list.remove(
      p.dataList,
      f.reduceRemove,
      path
    );
  ...
  if (p.exceptionAccessor) {
    f.reduceAdd = reductio_value_count.add(
      p.exceptionAccessor,
      f.reduceAdd,
      path
    );
    f.reduceRemove = reductio_value_count.remove(
      p.exceptionAccessor,
      f.reduceRemove,
      path
    );
  ...
  if (p.histogramValue && p.histogramThresholds) {
    ...
    f.reduceRemove = reductio_histogram.remove(
      p.histogramValue,
      f.reduceRemove,
      path
    );
    f.reduceInitial = reductio_histogram.initial(
      p.histogramThresholds,
      f.reduceInitial,
      path
    );
  }
  ...
  if (p.sumOfSquares) {
    ...
    f.reduceRemove = reductio_sum_of_sq.remove(
      p.sumOfSquares,
      f.reduceRemove,
      path
    );
  ...
  if (p.std) {
    if (!p.sumOfSquares || !p.sum) {
      console.error(
        "You must set .sumOfSq(accessor) and define a .sum(accessor) to use .std(true). Or use .std(accessor)."
      );
    } else {
  ...
  if (p.custom) {
    ...
    f.reduceRemove = reductio_custom.remove(
      f.reduceRemove,
      path,
      p.custom.remove
    );
    f.reduceInitial = reductio_custom.initial(
      f.reduceInitial,
      path,
      p.custom.initial
    );
  ...
  // Alias functions
  if (p.aliasKeys) {
    f.reduceInitial = reductio_alias.initial(
      f.reduceInitial,
      path,
      p.aliasKeys
    );
  ...
  if (p.aliasPropKeys) {
    ...
    f.reduceRemove = reductio_alias_prop.add(
      p.aliasPropKeys,
      f.reduceRemove,
      path
    );
  ...
  if (p.filter && !p.value) {
    ...
    f.reduceRemove = reductio_filter.remove(
      p.filter,
      f.reduceRemove,
      origF.reduceRemove,
      path
...
var reductio_cap = function (prior, f, p) {
